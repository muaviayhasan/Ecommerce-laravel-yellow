        ...
        <template x-if="authed || started">
            <div class="flex-1 flex flex-col min-h-0">
                ...
                <form @submit.prevent="send()" class="p-3 border-t border-outline-variant/60 flex items-end gap-2 shrink-0 bg-white">
                    <textarea x-model="body" @keydown.enter.prevent="send()" rows="1" maxlength="5000" placeholder="Write a message…"
                        class="flex-1 resize-none border border-outline-variant rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#2563eb] focus:border-[#2563eb] max-h-24"></textarea>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('supportChat', (cfg) => ({
                    ...
                    ping() { try { const a = new Audio('/assets/bells/user_notif.mp3'); a.volume = 0.5; a.play().catch(() => {}); } catch (e) {} },
                    subscribe(token) {
                    ...
                    init() { this.refresh().then(() => { if (this.started) this.startPolling(); }); },
                    toggle() {
                        this.open = !this.open;
                    ...
                    async refresh() {
                        try {
                            const url = cfg.stateUrl + (cfg.stateUrl.includes('?') ? '&' : '?') + 'open=' + (this.open ? 1 : 0);
