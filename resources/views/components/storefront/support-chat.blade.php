    ...
    <div x-show="open" x-cloak x-transition.origin.bottom.right
        ...
        <div class="bg-primary-container text-on-primary-container px-5 py-4 flex items-center justify-between gap-3 shrink-0">
            <div class="flex items-center gap-3 min-w-0">
                <span class="w-10 h-10 rounded-full bg-black/10 grid place-items-center shrink-0"><span aria-hidden="true" class="material-symbols-outlined">support_agent</span></span>
                <div class="min-w-0">
                    <p class="font-semibold leading-tight">Support</p>
                    <p class="text-xs opacity-80 truncate">Typically replies in a few minutes</p>
                </div>
            </div>
            <button type="button" @click="open = false" class="w-8 h-8 grid place-items-center rounded-full hover:bg-black/10 transition shrink-0"><span aria-hidden="true" class="material-symbols-outlined text-[20px]">close</span></button>
        </div>

        {{-- WhatsApp quick-connect strip — mobile only. On desktop the launcher's
             channel picker already offered WhatsApp before the panel opened; on
             mobile the panel opens directly (header icon / drawer link), so the
             strip is where the choice lives. Hidden until a number is set in
             Admin → Settings → Store. --}}
        <a x-show="whatsapp" x-cloak :href="waUrl()" target="_blank" rel="noopener"
            class="group md:hidden flex items-center gap-3 px-5 py-3 bg-[#25D366] text-white shrink-0 hover:brightness-105 active:brightness-95 transition">
            ...
            <span aria-hidden="true" class="material-symbols-outlined text-[20px] text-white/90 group-hover:translate-x-0.5 transition-transform shrink-0">chevron_right</span>
        </a>
        ...
        <template x-if="!authed && !started">
            <form @submit.prevent="startChat()" class="flex-1 flex flex-col justify-center gap-4 p-6 text-center">
                <span aria-hidden="true" class="material-symbols-outlined text-primary mx-auto" style="font-size:44px;">chat</span>
                <div>
            ...
            <div class="flex-1 flex flex-col min-h-0">
                <div x-ref="scroll" @scroll.passive="onScroll($event)" class="flex-1 overflow-y-auto overscroll-contain p-4 space-y-3 bg-surface-container-low/40">
                    <div x-show="loadingMore" class="flex justify-center py-1">
                        <span aria-hidden="true" class="material-symbols-outlined animate-spin text-outline text-[18px]">progress_activity</span>
                    </div>
                            ...
                            <div :class="m.from_admin ? 'bg-white text-on-surface rounded-tl-none' : 'bg-primary-container text-on-primary-container rounded-tr-none'"
                                ...
                                <p :class="m.from_admin ? 'text-outline' : 'text-on-primary-container/70'" class="text-[10px] mt-0.5 flex items-center justify-end gap-0.5">
                                    ...
                                    <template x-if="!m.from_admin">
                                        <span aria-hidden="true" class="material-symbols-outlined text-[14px] leading-none"
                ...
                <div x-show="blocked" x-cloak class="p-4 border-t border-outline-variant/60 bg-error/5 text-center shrink-0">
                    <p class="text-xs text-error font-medium flex items-center justify-center gap-1">
                        <span aria-hidden="true" class="material-symbols-outlined text-[16px]">block</span>
                ...
                <form x-show="!blocked" @submit.prevent="send()" class="p-3 border-t border-outline-variant/60 flex items-end gap-2 shrink-0 bg-white">
                    ...
                    <button type="submit" :disabled="!body.trim() || sending" class="w-10 h-10 grid place-items-center rounded-lg bg-primary-container text-on-primary-container disabled:opacity-50 hover:brightness-110 transition shrink-0"><span aria-hidden="true" class="material-symbols-outlined text-[20px]">send</span></button>
                </form>
            </div>
        </template>
    </div>

    {{-- Channel picker: the launcher reveals one circle per way to reach us —
         WhatsApp (deep link) and the live chat. Skipped entirely when no
         WhatsApp number is configured; the launcher then opens the chat straight
         away, since a one-item menu is just an extra click. --}}
    <div x-show="menu" x-cloak x-transition.origin.bottom.right
        class="hidden md:flex flex-col items-end gap-2.5">
        <a :href="waUrl()" target="_blank" rel="noopener" @click="menu = false"
            class="group flex items-center gap-2.5">
            <span class="bg-white text-on-surface text-xs font-semibold px-3 py-1.5 rounded-full shadow-md border border-outline-variant/60">WhatsApp</span>
            <span class="w-12 h-12 rounded-full bg-[#25D366] text-white shadow-xl grid place-items-center group-hover:brightness-105 group-active:scale-95 transition">
                <svg viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6" aria-hidden="true"><path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.945C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 018.413 3.488 11.824 11.824 0 013.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.512 5.26l-.999 3.648 3.736-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
            </span>
        </a>
        <button type="button" @click="openChat()" class="group flex items-center gap-2.5">
            <span class="bg-white text-on-surface text-xs font-semibold px-3 py-1.5 rounded-full shadow-md border border-outline-variant/60">Live chat</span>
            <span class="w-12 h-12 rounded-full bg-primary-container text-on-primary-container shadow-xl grid place-items-center group-hover:brightness-110 group-active:scale-95 transition relative">
                <span aria-hidden="true" class="material-symbols-outlined">forum</span>
                <span x-show="unread" x-cloak class="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-error text-white text-[11px] font-bold grid place-items-center" x-text="unread"></span>
            </span>
        </button>
    </div>

    {{-- Launcher — desktop only; on mobile the header chat icon (and the
         drawer link) open the panel instead, keeping the thumb zone clear. --}}
    <button type="button" @click="fabClick()"
        ...
        <span aria-hidden="true" class="material-symbols-outlined" x-text="(open || menu) ? 'close' : 'support_agent'"></span>
        <span x-show="!open && !menu && unread" x-cloak class="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-error text-white text-[11px] font-bold grid place-items-center" x-text="unread"></span>
    </button>
</div>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('supportChat', (cfg) => ({
                    ...
                    // Launcher: open panel → close it; otherwise offer the channel picker,
                    // or go straight to the chat when WhatsApp isn't configured.
                    fabClick() {
                        if (this.open) { this.open = false; return; }
                        if (!this.whatsapp) { this.openChat(); return; }
                        this.menu = !this.menu;
                    },
                    openChat() {
                        this.menu = false;
