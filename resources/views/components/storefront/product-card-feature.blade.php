...
<div {{ $attributes->class('product-card group relative flex flex-col bg-white px-8 py-6 transition-all duration-200 hover:shadow-[0_0_6px_0_rgba(1,1,1,0.3)] hover:z-20') }}>
    ...
    <h4 class="text-base font-semibold text-on-surface mb-4 line-clamp-2">
        ...
        <div class="flex gap-3 mb-6">
            @foreach ($thumbnails as $thumb)
                <button type="button" aria-label="View image"
                    class="w-16 h-16 border border-outline-variant p-1 hover:border-primary-container transition-colors">
    ...
    <div class="mt-auto flex items-center justify-between">
        ...
        <x-storefront.add-to-cart :variant-id="$variantId" :in-stock="$inStock" :name="$name" :url="$url" icon-class="text-[18px]"
            class="w-9 h-9 rounded-full bg-surface-container text-secondary flex items-center justify-center transition-colors group-hover:bg-primary-container group-hover:text-on-primary-container hover:!bg-primary-fixed-dim" />
    </div>
    ...
    <div class="absolute left-0 right-0 top-full -mt-px bg-white px-8 pb-6 z-10 opacity-0 invisible translate-y-1 transition-all duration-200 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 shadow-[0_3px_6px_0_rgba(1,1,1,0.3)]">
        <div class="flex items-center justify-center gap-6 border-t border-outline-variant pt-4 text-label-sm font-medium text-on-surface-variant">
            @if ($slug)
