                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-label-sm text-on-surface-variant flex items-center gap-1 min-w-0">
                            ...
                            @if ($user->hasVerifiedEmail())
                                <span aria-hidden="true" class="material-symbols-outlined text-info text-[15px] shrink-0"
                ...
                <nav class="p-2">
                    @foreach ($nav as $key => $item)
                        <a href="{{ route($item['route']) }}" @class([
                            ...
                            <span aria-hidden="true" class="material-symbols-outlined text-[20px]">{{ $item['icon'] }}</span> {{ $item['label'] }}
                        </a>
                    @endforeach
            ...
            <div class="lg:col-span-3 space-y-6">
                ...
                @if (! $user->hasVerifiedEmail() && setting('emails', 'email_verification', true))
                    <div x-data="{ show: sessionStorage.getItem('hideVerifyBanner') !== '1' }" x-show="show" x-cloak
                        ...
                        <span aria-hidden="true" class="material-symbols-outlined text-primary shrink-0">mark_email_unread</span>
                        <div class="flex-1 min-w-0">
                            ...
                            <form method="POST" action="{{ route('verification.send') }}" class="mt-2">
                                ...
                                <button type="submit" class="inline-flex items-center gap-1.5 text-primary font-bold text-label-sm hover:underline">
                                    <span aria-hidden="true" class="material-symbols-outlined text-[18px]">outgoing_mail</span> Resend verification email
                                </button>
                            </form>
                        </div>
