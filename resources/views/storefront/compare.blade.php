        ...
        <div class="app-container">
            ...
            @if (session('status') || session('error'))
                <div class="mb-6 p-4 rounded-lg flex items-center gap-2 {{ session('error') ? 'bg-error-container/40' : 'bg-secondary-container/40' }} text-on-surface">
                    <span aria-hidden="true" class="material-symbols-outlined {{ session('error') ? 'text-error' : 'text-secondary' }}">{{ session('error') ? 'error' : 'check_circle' }}</span>
            ...
            @if ($rows->isEmpty())
                <div class="bg-white rounded-lg border border-outline-variant p-16 text-center">
                    <span aria-hidden="true" class="material-symbols-outlined text-outline-variant" style="font-size:72px;">sync</span>
                                    ...
                                    <td class="p-4 align-top border-l border-outline-variant" style="width: {{ 80 / max(1, $rows->count()) }}%">
                                        <div class="flex justify-between items-start gap-2 mb-3">
                                            ...
                                            <form method="POST" action="{{ route('compare.remove', $row['slug']) }}">@csrf @method('DELETE')
                                                <button type="submit" aria-label="Remove" class="p-1 text-on-surface-variant hover:text-error"><span aria-hidden="true" class="material-symbols-outlined text-[18px]">close</span></button>
                                            </form>
                                        </div>
                                    ...
                                    <td class="p-4 align-top border-l border-outline-variant">
                                        @if ($row['variant_id'])
                                            <form method="POST" action="{{ route('cart.add') }}">
                                                ...
                                                <button type="submit" class="bg-primary-container text-on-primary-container px-4 py-2 rounded text-label-sm font-bold hover:brightness-95 transition-all flex items-center gap-1"><span aria-hidden="true" class="material-symbols-outlined text-[16px]">shopping_cart</span> Add</button>
                                            </form>
                                        @endif
                                    </td>
                                @endforeach
