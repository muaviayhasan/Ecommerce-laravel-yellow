        ...
        <div class="app-container">
            ...
            @if ($products->isEmpty())
                <div class="bg-white rounded-lg border border-outline-variant p-16 text-center">
                    <span aria-hidden="true" class="material-symbols-outlined text-outline-variant" style="font-size:72px;">favorite</span>
                        ...
                        <div class="bg-white border border-outline-variant rounded-lg overflow-hidden flex flex-col group">
                            <div class="relative">
                                ...
                                <form method="POST" action="{{ route('wishlist.remove', $product['slug']) }}" class="absolute top-2 right-2">
                                    @csrf @method('DELETE')
                                    <button type="submit" aria-label="Remove" class="w-8 h-8 grid place-items-center rounded-full bg-white shadow border border-outline-variant text-on-surface-variant hover:text-error"><span aria-hidden="true" class="material-symbols-outlined text-[18px]">close</span></button>
                                </form>
                            </div>
                                ...
                                <div class="mt-auto flex items-center justify-between gap-2 pt-2">
                                    ...
                                    @if ($product['variant_id'])
                                        <form method="POST" action="{{ route('cart.add') }}">
                                            ...
                                            <button type="submit" class="bg-primary-container text-on-primary-container px-3 py-2 rounded text-label-sm font-bold hover:brightness-95 transition-all flex items-center gap-1"><span aria-hidden="true" class="material-symbols-outlined text-[16px]">shopping_cart</span> Add</button>
                                        </form>
                                    @endif
                                </div>
                            </div>
