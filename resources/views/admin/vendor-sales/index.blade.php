                ...
                <form method="POST" action="{{ route('admin.vendor-sales.store') }}" @submit="if (!cart.length || !customerId) $event.preventDefault()"
                    ...
                    <x-admin.panel title="Checkout">
                        ...
                        <div class="mt-4 space-y-1.5">
                            ...
                            <input type="number" step="any" min="0" x-model="paid" placeholder="0.00" data-nav class="w-full bg-surface-container-low border border-outline-variant rounded-lg px-3 py-2 text-sm text-on-surface outline-none focus:ring-1 focus:ring-primary">
                        ...
                        <button type="submit" data-nav :disabled="!cart.length || !customerId" :class="(cart.length && customerId) ? '' : 'opacity-50 cursor-not-allowed'"
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('vendorSale', (cfg) => ({
                ...
                draftLabel() {
                    ...
                    return (c ? c.name : 'Vendor') + ' · ' + new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                },
                async loadDrafts() {
                    try {
