...
@section('title', 'Edit info bar item')

@section('content')
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-label-sm mb-1">
                <a href="{{ route('admin.info-bar-items.index') }}" class="text-primary font-semibold hover:underline">Info Bar</a>
                <span class="material-symbols-outlined text-outline text-[16px]">chevron_right</span>
                <span class="text-on-surface-variant font-semibold truncate">{{ $item->title }}</span>
            </div>
            <h2 class="text-2xl font-bold text-on-surface">Edit info bar item</h2>
        </div>
    ...
    <form method="POST" action="{{ route('admin.info-bar-items.update', $item) }}" class="space-y-6">
        @csrf
        @method('PUT')
        @include('admin.info-bar-items._form')

        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.info-bar-items.index') }}"
