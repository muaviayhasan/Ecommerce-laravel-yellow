...
@section('content')
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-label-sm mb-1">
                ...
                <span class="text-on-surface-variant font-semibold truncate">{{ $card->title }}</span>
            </div>
            <h2 class="text-2xl font-bold text-on-surface">Edit promo card</h2>
        </div>
    ...
    <form method="POST" action="{{ route('admin.promo-cards.update', $card) }}" class="space-y-6">
        @csrf
        @method('PUT')
